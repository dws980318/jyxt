...
export default {
  ...
  methods: {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let teacherInioVO = {
            ...
            ti_qq: this.ruleForm.qq,
            ti_othernumber: this.ruleForm.number,
            logurl: this.logurl,
            userconent: this.content
          };
          this.api.updTeacherInioBySuId(teacherInioVO).then(res => {
            console.log(res);
            if (res.code === 200) {
              this.$message({
                message: res.inio,
    ...
    seleUser() {
      ...
      this.api.seleTeacherAccountByUserId(teacherInioVO).then(res => {
        console.log(res);
        if (res.code === 200) {
          this.ruleForm.account = res.data.account;
          this.ruleForm.name = res.data.realname;
          this.ruleForm.phone = res.data.phonenumber;
          this.ruleForm.qq = res.data.ti_qq;
          this.ruleForm.number = res.data.ti_othernumber;
